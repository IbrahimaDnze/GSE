  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await api.post('/auth/register', {
        ...
        role: form.role === 'Administrateur' ? 'admin' :
              form.role === 'Directeur' ? 'directeur' :
              form.role === 'Enseignant' ? 'enseignant' :
              form.role === 'Comptable' ? 'comptable' : 'enseignant',
    ...
    <div className="min-h-screen flex bg-slate-100">
      ...
      >
        ...
        <div className="relative z-10 text-center">
          ...
          <p className="text-emerald-200 mt-2 text-base font-medium">Créez votre espace scolaire</p>
            ...
            {steps.map((s, i) => (
              <div key={s.n} className="flex items-center flex-1">
                <div className="flex flex-col items-center">
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold transition-all ${step > s.n ? 'bg-emerald-500 text-white' : step === s.n ? 'bg-primary-600 text-white shadow-sm shadow-green-900/20' : 'bg-slate-200 text-slate-400'}`}>
                    {step > s.n ? (
                      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M5 13l4 4L19 7" />
                      </svg>
              ...
              {step === 2 && (
                <div className="space-y-4">
                  ...
                  <div className="bg-emerald-50 border border-emerald-100 rounded-xl p-4">
                    <p className="text-xs font-semibold text-emerald-700 mb-1">💡 Bon à savoir</p>
                    <p className="text-xs text-emerald-600/80">Vous pourrez modifier ces informations à tout moment depuis les paramètres.</p>
                  </div>
                </div>
              )}
