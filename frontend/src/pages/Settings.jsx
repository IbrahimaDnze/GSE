...
const Settings = () => {
  ...
  const fetchUsers = async () => {
    ...
    try {
      ...
      setUsers(res.data.map(u => ({
        ...
        id: u._id || u.id,
        name: u.nom || u.name,
        active: u.actif !== undefined ? u.actif : u.active,
      })));
    } catch {
      // ignore
    }
  ...
  const handleUserSubmit = async (e) => {
    ...
    try {
      if (editingUser) {
        const updates = { nom: userForm.name.trim(), email: userForm.email.trim(), role: userForm.role, actif: userForm.active };
        if (userForm.password) updates.motDePasse = userForm.password;
        await api.put(`/users/${editingUser.id}`, updates);
      } else {
        await api.post('/users', { nom: userForm.name.trim(), email: userForm.email.trim(), role: userForm.role, motDePasse: userForm.password, actif: userForm.active });
      }
      await fetchUsers();
      showToast(editingUser ? 'Utilisateur modifié avec succès' : 'Utilisateur créé avec succès', 'success');
    } catch (err) {
      setUserFormError(err.response?.data?.message || 'Erreur lors de la sauvegarde.');
  ...
  const handleChangePassword = async () => {
    ...
    try {
      await api.put('/auth/profile', { motDePasse: pwForm.newPw, motDePasseActuel: pwForm.current });
  ...
  const handleSave = async () => {
    try {
      await updateSettings({ ...schoolForm });
      setSaved(true);
            ...
            {TABS.map(t => (
              <button key={t.id} onClick={() => setTab(t.id)}
                className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-colors ${
                    tab === t.id ? 'bg-primary-600 text-white shadow-sm' : 'text-slate-600 hover:bg-slate-100'
                }`}>
                <span>{t.icon}</span>
                <span>{t.label}</span>
              </button>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-card">
                    ...
                    <div className="flex items-start gap-5">
                      ...
                      <div
                        ...
                        className={`flex-1 border-2 border-dashed rounded-2xl p-6 flex flex-col items-center justify-center gap-2 cursor-pointer transition-all ${
                          dragOver ? 'border-primary-400 bg-emerald-50' : 'border-slate-200 hover:border-primary-400 hover:bg-emerald-50/50'
                        }`}>
                        ...
                        <button type="button" onClick={(e) => { e.stopPropagation(); fileInputRef.current?.click(); }}
                          className="mt-1 px-4 py-1.5 bg-primary-600 text-white text-xs font-semibold rounded-lg hover:bg-primary-700 transition-colors shadow-sm">
                          Parcourir
                        </button>
                      </div>
            ...
            <div className="px-6 py-4 border-t border-slate-100 flex justify-end gap-3">
              ...
              <button onClick={handleSave}
                className="inline-flex items-center gap-2 px-5 py-2 bg-primary-600 text-white rounded-xl text-sm font-semibold hover:bg-primary-700 transition-colors shadow-sm">
